...
public class MoveSelect {
    ...
    public static PentagoMove calcBestEarlyGameMove(int playerID, PentagoBoardState pbs) {
        ...
        if ((q1.compareTo(Piece.EMPTY) == 0 || q1.compareTo(Piece.values()[1 - playerID]) == 0) && (q3.compareTo(Piece.EMPTY) == 0) || q3.compareTo(Piece.values()[1 - playerID]) == 0) {
            ...
            if (q1.compareTo(Piece.EMPTY) == 0) {
                availability.add(new int[]{4, 1});
            }
            if (q3.compareTo(Piece.EMPTY) == 0) {
                availability.add(new int[]{1, 4});
            }
        }
        //Attempt to get Second Diagonal Setup (1,1) (4,4)
            ...
            if (q0.compareTo(Piece.EMPTY) == 0) {
                availability.add(new int[]{1, 1});
            }
            if (q2.compareTo(Piece.EMPTY) == 0) {
                availability.add(new int[]{4, 4});
            }
        } else {
    ...
    public static PentagoMove calcEarlyAttack(int playerID, PentagoBoardState pbs) {
        ...
        if (q1 && q3) {
            ...
            if (pbs.getPieceAt(2, 5).compareTo(Piece.EMPTY) == 0) {
                availability.add(new int[]{2, 5});
            }
            if (pbs.getPieceAt(3, 0).compareTo(Piece.EMPTY) == 0) {
                availability.add(new int[]{3, 0});
            }
        } else if (q0 && q2) { //TODO: Fix this, this if statement is wrong. if the best positions cant be obtained, go for positions VVV
            System.out.println("Second Best Diagonal Setup - Attack");
            if (pbs.getPieceAt(5, 2).compareTo(Piece.EMPTY) == 0) {
                availability.add(new int[]{5, 2});
            }
            if (pbs.getPieceAt(0, 3).compareTo(Piece.EMPTY) == 0) {
                availability.add(new int[]{0, 3});
            }
        } else { //Todo: Add if statement for if q0 and q2 is obtained, then aim for either (0,2) or (3,5)
            ...
            for (int i = 0; i < 3; i++) {
                for (int j = 0; j < 3; j++) {
                    if (q0) {
                        if (pbs.getPieceAt(i, j).compareTo(Piece.EMPTY) == 0) {
                            availability.add(new int[]{i, j});
                        }
                    }
                    if (q1) {
                        if (pbs.getPieceAt(i + 3, j).compareTo(Piece.EMPTY) == 0) {
                            availability.add(new int[]{i + 3, j});
                        }
                    }
                    if (q2) {
                        if (pbs.getPieceAt(i, j + 3).compareTo(Piece.EMPTY) == 0) {
                            availability.add(new int[]{i + 3, j + 3});
                        }
                    }
                    if (q3) {
                        if (pbs.getPieceAt(i + 3, j + 3).compareTo(Piece.EMPTY) == 0) {
                            availability.add(new int[]{i, j + 3});
                        }
                    }
                }
            }
